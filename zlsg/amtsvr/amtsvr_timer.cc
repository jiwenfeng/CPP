...
struct timer {
	struct link_list _near[TIME_NEAR];
...
add_node(struct timer *T,struct timer_node *node)
{
	...
	if ((time|TIME_NEAR_MASK)==(current_time|TIME_NEAR_MASK)) {
		link(&T->_near[time&TIME_NEAR_MASK],node);
	}
	else {
		int i;
...
timer_execute(struct timer *T)
{
	...
	while (T->_near[idx].head.next) {
		current=link_clear(&T->_near[idx]);
		
		do {
			struct timer_event * event = (struct timer_event *)(current+1);
			struct amtsvr_message message;
			message.source = 0;
			message.session = event->session;
			message.data = NULL;
			message.sz = PTYPE_TIMER << HANDLE_REMOTE_SHIFT;

			amtsvr_context_push(event->handle, &message);
...
int
amtsvr_timeout(uint32_t handle, int time, int session) {
	if (time == 0) {
		struct amtsvr_message message;
		message.source = 0;
		...
		if (amtsvr_context_push(handle, &message)) {
			return -1;
		}
	} else {
...
{
    while (__sync_lock_test_and_set(&TI->lock,1)) {};

    struct timer_node* current;
    for (int i = 0; i < TIME_NEAR; ++i) {
        if (!TI->_near[i].head.next) {
            continue;
        }

        current = link_clear(&TI->_near[i]);
        if (current == NULL) {
            continue;
        }
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < (TIME_LEVEL - 1); ++j) {
            current = link_clear(&TI->t[i][j]);
            if (current == NULL) {
                continue;
            }
