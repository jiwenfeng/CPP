...
client_push(struct send_client *c, void * buf, int sz, void * ptr) {
    struct send_buffer * sb = (struct send_buffer *)malloc(sizeof(*sb));
    sb->next = NULL;
    sb->buff = (char * )buf;
    sb->size = sz;
    sb->ptr = (char * )ptr;
...
#else
            int sz = send(s->fd, (const char*)buffer, size, 0);
#endif // _WIN32
            if (sz < 0) {
                switch(errno) {
                case EINTR:
                ...
                default:
                    ...
                    try_close(self, s);
                    //printf("-----------------------mread_push write error!errno=%d\n", errno);
                    return;
                }
                break;
            }
...
static struct socket* _create_sockets(int max) {
    int i;
    struct socket * s = (struct socket *)malloc(max * sizeof(struct socket));
    for (i=0;i<max;i++) {
        s[i].fd = i+1;
        s[i].node = NULL;
        s[i].temp = NULL;
...
{
    u_long iMode = 0;
    int flag = ioctlsocket(fd, FIONBIO, &iMode);
    if (flag != NO_ERROR) {
...
mread_create(uint32_t addr, int port , int max , int buffer_size) {
    ...
    int reuse = 1;
    setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(int));
    ...
    struct mread_pool * self = (struct mread_pool *)malloc(sizeof(*self));
...
#else
        shutdown(self->listen_fd, 0);
#endif // _WIN32
        self->listen_fd = 0;
    }
}

static int _read_queue(struct mread_pool * self, int timeout) {
    self->queue_head = 0;
...
static void force_close_client_ex(struct mread_pool * self, struct socket * s) {
    free_buffer(&s->client);
    if (s->status >= SOCKET_ALIVE && s->status != SOCKET_HALFCLOSE) {
        s->status = SOCKET_HALFCLOSE;
        try_close(self, s);
        //printf("-----------------------force_close_client_ex s->status=%d\n", s->status);
    }
}

static void force_close_client(struct mread_pool * self, int id) {
    force_close_client_ex(self, &self->sockets[id]);
}

inline static struct socket * _read_one(struct mread_pool * self) {
    for (;;) {
        if (self->queue_head >= self->queue_len) {
            return NULL;
        }
        struct socket * ret = (struct socket *)self->ev[self->queue_head].s;
        bool writeflag = self->ev[self->queue_head].write;
        bool readflag = self->ev[self->queue_head].read;
        ++ self->queue_head;
        if (ret == LISTENSOCKET || ret == NULL) {
            return ret;
        }
        ...
        if (writeflag && ret->status != SOCKET_CLOSED) {
            ...
            try_close(self, ret);
            //printf("-----------------------_read_one error s->status=%d\n", ret->status);
        }
...
mread_close_client(struct mread_pool * self, int id) {
    ...
    if (s->status >= SOCKET_ALIVE && s->status != SOCKET_HALFCLOSE) {
        s->status = SOCKET_HALFCLOSE;
        try_close(self, s);
        //printf("-----------------------mread_close_client error s->status=%d\n", s->status);
    }
}
