...
ringbuffer_new(int size) {
	struct ringbuffer * rb = (struct ringbuffer *)malloc(sizeof(*rb) + size);
	rb->size = size;
	rb->head = 0;
...
_alloc(struct ringbuffer * rb, int total_size , int size) {
	...
	if (next) {
		...
		if (align_length < total_size) {
			...
			if (next->length >= (int)sizeof(struct ringbuffer_block)) {
				next->id = -1;
			}
		}
	} else {
...
ringbuffer_alloc(struct ringbuffer * rb, int size) {
	...
	for (i=0;i<2;i++) {
		...
		struct ringbuffer_block * blk = block_ptr(rb, rb->head);
		do {
			if (blk->length >= (int)sizeof(struct ringbuffer_block) && blk->id >= 0)
				return NULL;
...
ringbuffer_collect(struct ringbuffer * rb) {
	...
	struct ringbuffer_block *blk = block_ptr(rb, 0);
	do {
		if (blk->length >= (int)sizeof(struct ringbuffer_block) && blk->id == id) {
			blk->id = -1;
		}
		blk = block_next(rb, blk);
	} while(blk);
...
ringbuffer_shrink(struct ringbuffer * rb, struct ringbuffer_block * blk, int size) {
	...
	if (blk->length >= (int)sizeof(struct ringbuffer_block)) {
		blk->id = -1;
	}
	rb->head = block_offset(rb, blk);
}
...
_block_id(struct ringbuffer_block * blk) {
	assert(blk->length >= (int)sizeof(struct ringbuffer_block));
	int id = blk->id;
	assert(id>=0);
	return id;
}
