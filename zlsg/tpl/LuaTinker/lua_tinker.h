...
//		luaL_openlibs( L );
	...
	inline void open_string( lua_State *L )
	{
#if	( LUA_VERSION_NUM >= 502 )
	...
	inline void open_coroutine( lua_State *L )
	{
#if	( LUA_VERSION_NUM >= 502 )
	...
	inline void open_os( lua_State *L )
	{
#if	( LUA_VERSION_NUM >= 502 )
	...
	inline void open_io( lua_State *L )
	{
#if	( LUA_VERSION_NUM >= 502 )
	...
	struct lua_context
	{
		lua_context( void ) : L( lua_tinker::open() )
		{
		}
		~lua_context( void )
		{
...
//		lua_settable(L, LUA_GLOBALSINDEX);
	...
	template<typename T>
		const char* class_name<T>::m_name = "";
	...
	struct table_obj
	{
		...
		RVal call(const char* name)
		{
			...
			{
				print_error(m_L, "lua_tinker::call() attempt to call table `%s' (not a function)", name);
			}
