...
class PBParser
{
    ...
    // different typess
    var $scalar_types = array('double' 	 => 'PB64Bit',
    						  'float'	 => 'PB32Bit',
    						  'int32' 	 => 'PBInt',
    						  'int64' 	 => 'PBInt',
                              'uint32' 	 => 'PBInt',
    ...
    { 	    
        ...
        if (empty($outputfile))
        	$outputfile = dirname(dirname(__FILE__)).'/pb2php/pb_proto_' . $name . '.php';
    ...
    private function _parse_message_type(&$string, $m_name, $path = '')
    {
        ...
        while (strlen($string) > 0)
        {
            ...
            else if (strtolower($next) == 'enum')
            {
                ...
                $this->m_types[$newName] = array('name' => $newName,
            ...
            else if( strtolower($next) == 'import' )
            {
                ...
                $fn = dirname(dirname(__FILE__)).'/proto/'.$matches[1][0];
                if( !file_exists($fn) )
                {
        ...
        $this->m_types[$path] =  array('name' => $path , 'type' => 'message', 'value' => $myarray);
    }
    ...
    private function _parse_enum($content)
    {
        ...
        foreach ($matches[1] as $match)
        {
            ...
            $myarray[] = array(trim($explode[0]), trim($explode[1]));
        }
        return $myarray;
    }
