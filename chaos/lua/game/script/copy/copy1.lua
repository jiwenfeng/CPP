...
local MOPUP_TIMELONG = 180
local MOPUP_NEED_GRID = 2
local HERO_COPY_BUY_PRICE = 5		--英魂副本购买价格基数
local STATE_DOING = 1
local STATE_FINISH = 0
local FAST_MOPUP_NEED_INGOT = 2
local HERO_FAST_MOPUP = 80
local NORMAL_FAST_MOPUP = 60
...
function CopyMopUp(user, copyId, times)
	...
	if info.type == "hero" then
		...
		if VIP.IsVipFuncValid(user:GetVipLv(), VIP.VIP_HERO_COPY_MOPUP_FINISH) or user:GetLv() >= HERO_FAST_MOPUP then
			flag = true
		end
	else
		user:SetVigour(-NORMAL_COPY_COST_VIGOUR)
		if VIP.IsVipFuncValid(user:GetVipLv(), VIP.VIP_NORMAL_COPY_MOPUP_FINISH) or user:GetLv() >= NORMAL_FAST_MOPUP then
			flag = true
		end
	end
...
function GetHeroCopyTimesInfo(user)
	...
	buff.buyTimes = VIP.GetVipFuncValidTimes(user:GetVipLv(), VIP.VIP_HERO_COPY_ADD_TIMES) + USERCOPY.DAILY_HERO_COPY_NUM - user:GetDailyHeroCopyTimes("max")
...
local s_copy_list = function(connection, data)
	...
	local sData = {}
--	sData.selectMap = info.selectMap
	sData.mopupLv = MOPUP_MIN_LV
	sData.mopupNeed = MOPUP_COST_GOLD
	sData.list = GetFormatedCopyList(user, townId, info.copyType)
...
local s_copy_teamList = function(connection, data)
	if data ~= "get" then
		return ERROR.TipMsg(connection, "无效的请求")
	end
	local user = assert(USER.GetOnlineUser(connection.roleId))
	...
	sData.type = "team"
	connection:SendProtocol("c_copy_teamList", sData)
end
...
local s_copy_enter = function(connection, data)
	...
	sData.groundList = copy:GetGoundItemList()
...
local s_heroCopy_buy = function(connection, data)
	...
	local user = assert(USER.GetOnlineUser(connection.roleId))
	if user:GetDailyHeroCopyTimes("max") >= USERCOPY.DAILY_HERO_COPY_NUM + VIP.GetVipFuncValidTimes(user:GetVipLv(), VIP.VIP_HERO_COPY_ADD_TIMES) then 
