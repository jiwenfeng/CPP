--local LOGIN_VERIFY_URL = "http://www.shangguzhange.com/proving.php?&username=%s&ytpwd=%s"
local LOGIN_VERIFY_URL = "http://192.168.0.20/chaosweb/proving.php?&username=%s&ytpwd=%s"
...
local s_user_register = function(connection, data)
	return connection:SendProtocol("c_user_register", {result = false, errorMsg = "请前往网站进行注册。"})
--[[	if type(data) ~= "table" then
		print("s_user_register", data or "null")
		return
	end
...
local s_verify = function(connection, data)
	...
	local function verify_request()
		local content = HTTP.CoRequest(string.format(LOGIN_VERIFY_URL, data.username, data.pwd))
		if not content then
			return connection:SendProtocol("c_verify_error", "no_respond")
		end
		local result = string.match(content, "[%c%p%s%z]*([%w_]*)$")
		if result ~= "true" then
			return connection:SendProtocol("c_verify_error", result)
		end
