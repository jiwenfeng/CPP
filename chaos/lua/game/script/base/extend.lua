...
function table.overlay(src, rel)
	local rel = rel or {}
	if type(src) ~= "table" then
		return rel
	end
...
function IsPosInt(k)
    return type(k)=='number' and math.floor(k)==k and 1<=k
end
...
--获取距离下一个 hour min 的秒数
function CalDiffSeconds(hour, min)
	local tm = os.date("*t")
	tm.hour = hour or 0
	tm.min = min or 0
	tm.sec = 0
	local sec = os.time(tm) - os.time()
	return sec >= 0 and sec or (24 * 3600 + sec)
end

--计算date1与date2之间相差的天数
function CalDiffDays(time1, time2, hour, min)
	local ONE_DAY = 24 * 3600
	local function adjust_time(time)
		local tm = os.date("*t", time)
		if hour then
			if tm.hour > hour then
				tm.day = tm.day + 1
			elseif tm.hour == hour and (not min or tm.min >= min) then
				tm.day = tm.day + 1
			end
		end
		tm.hour, tm.min, tm.sec = hour or 0, min or 0, 0
		return os.time(tm)
	end
	local diffTime = adjust_time(time2 or os.time()) - adjust_time(time1 or os.time())
	local nday = diffTime / ONE_DAY
	return nday
end

--获取UTF-8字符串长度(cnt:实际独立字符数量 cnChar:多字节的字符数量)
function utfstrlen(str)
	local len = #str
	local left = len
	local cnt, cnChar = 0, 0
