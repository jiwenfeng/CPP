function ParseTable(table)
	...
	for line, Data in ipairs(table[1]) do --只读第一页
		...
		if line > 1 then
			...
			if Data[7] then
				CodeString = CodeString .. string.format("\t\tmapId = \"%s\",\n", Data[7])
			end
			if Data[8] then
				CodeString = CodeString .. string.format("\t\tmonsterId = \"%s\",\n", Data[8])
				CodeString = CodeString .. string.format("\t\tmonsterPos = {x = %d, y = %d},\n", Data[9], Data[10])
			end
			CodeString = CodeString .. string.format("\t},\n")
		end
	end
...
function gen()
	local excelfile = "table/uplv_retreat.xls"
	local outfile = "autocode/uplv_retreat.lua"
