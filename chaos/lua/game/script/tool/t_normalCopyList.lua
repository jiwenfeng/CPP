function ParseTable(table)
	...
	for line, Data in ipairs(table[1]) do --只读第一页
			...
			for i = 0, 2 do
				if Data[10 + i * 14] then
					...
					if Data[21 + i * 14] then
						...
						if Data[20 + i * 14] then
							CodeString = CodeString .. string.format("needMissionId = \"%d\", ", Data[20 + i * 14])
						end
		...
		--[[	if Data[61] then
			...
			for i = 0, 3 do
				if Data[56 + i * 3] then
					CodeString = CodeString .. string.format("\t\t\t{id = %d, num = %d, rate = %g},\n", Data[56 + i * 3], Data[57 + i * 3], Data[58 + i * 3])
				end
			end
			CodeString = CodeString .. string.format("\t\t},\n")
			CodeString = CodeString .. string.format("\t\tshow = {\n")
			CodeString = CodeString .. string.format("\t\t\tid = %d,\n", Data[68])
...
function gen()
	local excelfile = "table/normalCopyList.xls"
	local outfile = "autocode/normalCopyList.lua"
