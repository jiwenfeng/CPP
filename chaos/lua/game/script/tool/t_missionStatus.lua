...
function ParseTable(table)
	...
	for line, Data in ipairs(table[1]) do --只读第一页
		if line > 1 then
			...
			if Data[9] then
				CodeString = CodeString .. string.format("\t\tneedStatusLv = %d,\n", Data[9])
			end
			if Data[10] then
			...
			for i = 0, 1 do
				if Data[13 + i * 5] then
					CodeString = CodeString .. string.format("\t\t\t{type = \"%s\"", Data[13 + i * 5])
					if Data[14 + i * 5] then
						CodeString = CodeString .. string.format(", mapId = %d", Data[14 + i * 5])
					end
					if Data[15 + i * 5] then
						CodeString = CodeString .. string.format(", npcId = \"%s\"", Data[15 + i * 5])
					end
					if tostring(Data[13 + i * 5] == "func") then
						if Data[16 + i * 5] then
							CodeString = CodeString .. string.format(", id = %d",Data[16 + i * 5])
						end
					else
						if Data[16 + i * 5] then
							CodeString = CodeString .. string.format(", id = \"%s\"",Data[16 + i * 5])
						end
					end
...
function gen()
	local excelfile = "table/missionStatus.xls"
	local outfile = "autocode/missionStatus.lua"
