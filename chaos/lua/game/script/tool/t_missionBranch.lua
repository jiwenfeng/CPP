...
function ParseTable(table)
	...
	for line, Data in ipairs(table[1]) do --只读第一页
		if line > 1 then
			...
			for i = 0, 1 do
				if Data[12 + i * 6] then
					CodeString = CodeString .. string.format("\t\t\t{type = \"%s\"", Data[12 + i * 6])
					if Data[13 + i * 6] then
						CodeString = CodeString .. string.format(", mapId = %d", Data[13 + i * 6])
					end
					if Data[14 + i * 6] then
						CodeString = CodeString .. string.format(", npcId = \"%s\"", Data[14 + i * 6])
					end
					if Data[15 + i * 6] then
						CodeString = CodeString .. string.format(", id = %d",Data[15 + i * 6])
					end
					if Data[16 + i * 6] then
						CodeString = CodeString .. string.format(", limit = %d",Data[16 + i * 6])
					end
...
function gen()
	local excelfile = "table/missionBranch.xls"
	local outfile = "autocode/missionBranch.lua"
