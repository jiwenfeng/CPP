...
static void lua_append_bson(lua_State *L, const char *key, int stackpos, BSONObjBuilder *builder) {
    ...
    if (type == LUA_TTABLE) {
        ...
        if (!bsontype_found) {
		    ...
		    if (lua_type(L, -2) == LUA_TNUMBER){
			    ...
			    /*
                        ...
                        lua_append_bson(L, ss.str().c_str(), -1, &b);
			*/
			luaL_error(L, LUAMONGO_UNSUPPORTED_LUA_TYPE, "all key must be string", lua_tostring(L, -2));
                    } else {
        ...
        if (numval == floor(numval)) {
            ...
            builder->append(key, static_cast<long long int>(numval));
        } else {
        ...
        luaL_error(L, LUAMONGO_UNSUPPORTED_LUA_TYPE, luaL_typename(L, stackpos), lua_tostring(L, stackpos));
    }
}
